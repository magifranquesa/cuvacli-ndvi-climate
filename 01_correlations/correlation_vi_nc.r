# ================================================================================
# CORRELACIONES MENSUALES ENTRE ÍNDICES DE VEGETACIÓN Y VARIABLES CLIMÁTICAS
# ================================================================================
# Para cada parque, índice (NDVI, kNDVI), variable climática (tmax, tmin, pr),
# escala de acumulación (1, 3, 6, 9, 12 meses) y mes del calendario, calcula en
# cada píxel la correlación interanual (1984-2023) entre el índice y la variable
# acumulada, una vez eliminada la tendencia de ambas series.
#
# Procedimiento por píxel y mes:
#   1. Se enmascaran los años en que falta el índice o la variable.
#   2. A las dos series restantes se les quita la tendencia lineal ajustada por
#      mínimos cuadrados contra el año real (no contra la posición en la serie),
#      de modo que los años ausentes no deforman la pendiente.
#   3. Pearson r sobre los residuos; p-valor bilateral con n-3 grados de libertad
#      (uno menos que n-2 por la tendencia estimada en cada serie).
#   4. Spearman rho sobre los mismos residuos en una submuestra reproducible de
#      píxeles (FRAC_SPEARMAN), para comprobar la sensibilidad al supuesto de
#      linealidad.
#
# La rejilla climática (1,1 km, EPSG:23030) se reproyecta, recorta al parque y
# remuestrea bilinealmente a la rejilla del índice (30 m). En los parques que la
# rejilla climática no cubre por completo (parques_gapfill) las celdas ausentes
# se rellenan antes con una media focal 3x3 iterativa.
#
# Salida: un NetCDF por parque, índice, variable y escala, con 12 bandas (meses):
#   correlation, p_value, significance (2/-2: p<=0.05 sin corregir; 1/-1: no),
#   n_obs, correlation_spearman, p_value_spearman.
# El umbral de años válidos y el control del FDR se aplican después, en
# 03_fdr/, sobre el p_value que aquí se guarda.
#
# Entradas: MOSAICOS/mosaicos_all/<parque>_<INDICE>_filled_clean.nc
#           CLIMA/agregados_lag/<var>_scale_<s>.nc
#           limites_red/.../Limites_PN.shp
# Salida  : articulo_2/correlaciones_v2/<parque>/correlation_<indice>_<var>_scale_<s>_<parque>.nc
# ================================================================================
...
# --- Igual, con relleno focal previo de las celdas climáticas ausentes ---
...
# Spearman en una submuestra de píxeles (la misma para todas las variables,
# escalas y meses del parque). FRAC_SPEARMAN <- 1 lo calcula en todos.
...
for (parque in parques) {
  ...
  for (vi in index_list) {
    ...
    # --- Aplicar máscara del parque desde shapefile general ---
    ...
    for (var in var_list) {
      cat("\nProcesando variable:", var, "para índice", vi, "\n")
      clim_file_template <- paste0(path, var, '_scale_')
      clima_proj_list <- list()

      for (scale in c(1, 3, 6, 9, 12)) {
        clim_file <- paste0(clim_file_template, scale, '.nc')
        clima_stack <- rast(clim_file)
        clima_resampled <- funcion_ext(clima_stack, parque_vect, vi_rast)
        clima_proj_list[[scale]] <- clima_resampled
      }

      for (scale in c(1, 3, 6, 9, 12)) {
        cat("  Escala:", scale, "mes(es)\n")
        var_data_subset <- as.array(clima_proj_list[[scale]])
        ...
        outfile <- file.path(out_path, paste0('correlation_', tolower(vi), '_', var, '_scale_', scale, '_', parque, '.nc'))
      ...
      cat("✔️ Variable procesada:", var, "para índice", vi, "\n")
    }
  }
}
