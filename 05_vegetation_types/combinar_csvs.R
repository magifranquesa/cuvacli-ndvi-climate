# ==============================================================================
# COMBINA LOS CSV POR VARIABLE EN UN ÚNICO CSV — CORRELACIONES POSITIVAS
# ==============================================================================
# Une los CSV que escriben snv_respuesta_clima_mensual_pos.R y
# snv_respuesta_clima_spei_mensual.R (pr, spei, tmax, tmin) en un solo fichero
# con una columna 'variable', que es
# el que leen los scripts de figuras de esta carpeta. Se detiene si falta
# alguno de los CSV de entrada.
#
# Entrada: resultados_v2/snv_respuesta_clima_mensual_positivo_<indice>_<var>.csv
# Salida : resultados_v2/snv_respuesta_clima_mensual_positivo_<indice>_completo.csv
# ==============================================================================
...
vars <- c("pr", "spei", "tmax", "tmin")
archivos <- setNames(
  sprintf("snv_respuesta_clima_mensual_positivo_%s_%s.csv", indice, vars),
  vars)
...
if (length(faltan)) {
  stop("Faltan estos CSV en ", dir_base, ":\n  ", paste(faltan, collapse = "\n  "),
       "\nEjecuta antes snv_respuesta_clima_mensual_pos.R y snv_respuesta_clima_spei_mensual.R",
       " con indice <- \"", indice, "\".")
}
...
df_combinado <- df_combinado %>%
  mutate(
    ...
    mes      = as.integer(mes),
    variable = factor(variable, levels = c("pr", "spei", "tmax", "tmin")),
...
# 💾 Guardar CSV final
output_csv <- file.path(dir_base, sprintf("snv_respuesta_clima_mensual_positivo_%s_completo.csv", indice))
