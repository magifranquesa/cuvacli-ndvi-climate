# ==============================================================================
# FIGURAS S9-S19: CORRELACIONES POSITIVAS SIGNIFICATIVAS POR TIPO DE
# VEGETACIÓN, UNA FIGURA POR PARQUE
# ==============================================================================
# Para cada parque, un panel por tipo de vegetación presente (coberturas_por_
# parque) con, para cada mes y variable (Pr, SPEI, Tmax, Tmin), el porcentaje
# de píxeles del tipo con correlación positiva significativa. Se dibuja el
# valor de cada escala de acumulación superpuesto (position_dodge sobre la
# variable), de modo que la barra visible corresponde a la escala con mayor
# fracción significativa en ese mes.
#
# Entrada: resultados_v2/snv_respuesta_clima_mensual_positivo_<indice>_completo.csv
#          (combinar_csvs.R)
# Salida : figuras/SNV_v2/fdr/por_parque_pos/snv_response_<parque>.{tif,pdf,svg}
# ==============================================================================
...
out_dir <- paste0(ROOT, "/articulo_2/figuras/SNV_v2/fdr/por_parque_pos")
...
for (i in 1:nrow(parques_info)) {
  ...
  # 💾 Guardar figura
  ggsave(
    file.path(out_dir, sprintf("snv_response_%s.tif", parque)),
    figura, width = width, height = height, dpi = 300
  )
  ggsave(
    file.path(out_dir, sprintf("snv_response_%s.pdf", parque)),
    figura, width = width, height = height
  )
  ggsave(
    file.path(out_dir, sprintf("snv_response_%s.svg", parque)),
    figura, width = width, height = height
  )
