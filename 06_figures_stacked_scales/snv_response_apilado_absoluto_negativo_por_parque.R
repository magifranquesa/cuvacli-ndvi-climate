# ==============================================================================
# FIGURAS S83-S115: CORRELACIONES NEGATIVAS MÁXIMAS POR TIPO DE VEGETACIÓN,
...
# porcentaje de píxeles del tipo con correlación negativa significativa,
...
# Salida  : figuras/figuras_mensuales_apilados_absolutos_SNV_v2/fdr/<indice>/por_parque_neg/
#             snv_response_<var>_scales_absoluto_negativo_<parque>.{tif,pdf,svg}
#           (el suplemento usa las de Pr, Tmax y Tmin; las de SPEI se generan pero no se incluyen)
# ==============================================================================
...
DIR_MAX <- paste0(ROOT, "/articulo_2/correlaciones_v2/maximos_cor_negative_fdr")
...
# 🚀 Bucle principal
# =============================
out_dir <- sprintf(paste0(ROOT, "/articulo_2/figuras/figuras_mensuales_apilados_absolutos_SNV_v2/fdr/%s/por_parque_neg"), indice)
...
for (variable in variables) {
  for (i in 1:nrow(parques_info)) {
    ...
    cat("\n📍 Procesando:", nombre_parque, "–", variable, "(negativas)\n")
    
    # Archivo NetCDF
    if (variable == "spei") {
      nc_path <- sprintf("%s/%s_max_neg_correlation_%s_%s.nc",
                         DIR_MAX, parque, variable, indice)
    } else {
      nc_path <- sprintf("%s/%s_max_neg_correlation_%s_%s_selscales.nc",
                         DIR_MAX, parque, variable, indice)
    }
    ...
    # Guardar figura
    ggsave(sprintf("%s/snv_response_%s_scales_absoluto_negativo_%s.tif", out_dir, variable, parque),
           figura, width=width, height=height, dpi=300)
    ggsave(sprintf("%s/snv_response_%s_scales_absoluto_negativo_%s.pdf", out_dir, variable, parque),
           figura, width=width, height=height)
    ggsave(sprintf("%s/snv_response_%s_scales_absoluto_negativo_%s.svg", out_dir, variable, parque),
           figura, width=width, height=height)
