# ==============================================================================
# FIGURAS S20-S23: CORRELACIONES POSITIVAS MÁXIMAS POR TIPO DE VEGETACIÓN,
# APILADAS POR ESCALA DE ACUMULACIÓN (red completa de parques)
# ==============================================================================
# Una figura por variable (SPEI, Pr, Tmax, Tmin) con un panel por tipo de
# vegetación: para cada mes, el porcentaje de píxeles del tipo con correlación
# positiva significativa, agregado sobre todos los parques y apilado por la
# escala en que se alcanza la correlación máxima. Cada píxel cuenta una sola
# vez.
#
# Entrada: resultados_v2/snv_respuesta_<var>_<indice>_absoluto_positivo.csv
#          (generar_csv_pos_clima.R, generar_csv_pos_spei.R)
# Salida : figuras/figuras_mensuales_apilados_absolutos_SNV_v2/fdr/<indice>/global_pos/
#            snv_response_<var>_ponderado_global.{tif,pdf,svg}
# ==============================================================================
...
out_base   <- sprintf(paste0(ROOT, "/articulo_2/figuras/figuras_mensuales_apilados_absolutos_SNV_v2/fdr/%s/global_pos"), indice)
...
# 🟢 Tipos de vegetación de interés (ordenados)
tipos_interes <- c(
...
generar_figura <- function(variable) {
  ...
  # 📁 CSV de entrada
  csv_path <- sprintf(paste0(ROOT, "/articulo_2/resultados_v2/snv_respuesta_%s_%s_absoluto_positivo.csv"), variable, indice)
  ...
  # 💾 Guardar figura
  out_path_tif <- sprintf("%s/snv_response_%s_ponderado_global.tif", out_base, variable)
  out_path_pdf <- sprintf("%s/snv_response_%s_ponderado_global.pdf", out_base, variable)
  out_path_svg <- sprintf("%s/snv_response_%s_ponderado_global.svg", out_base, variable)
...
walk(c("spei", "pr", "tmax", "tmin"), generar_figura)
